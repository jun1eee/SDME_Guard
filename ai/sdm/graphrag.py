...
from sdm.prompts import FEW_SHOT_EXAMPLES, RAG_TEMPLATE
...
class SdmGraphRagEngine:
    ...
    def ready(self) -> bool:
        return bool(
            ...
            and self.llm
            and self.embedder
            and self._vector_retriever_cls
            and self._graph_rag_cls
            and self._rag_template_cls
            and self._retriever_item_cls
        )
    ...
    def get_user_preference(self, couple_id: int) -> dict[str, Any]:
        ...
        try:
            ...
            cur.execute(
                "SELECT * FROM couple_preferences WHERE couple_id = %s",
                (couple_id,),
            )
            row = cur.fetchone()
            cur.close()
    ...
    def get_user_likes(self, couple_id: int) -> list[dict[str, Any]]:
        ...
        try:
            cur = self.mysql_conn.cursor(dictionary=True)
            cur.execute(
                "SELECT * FROM couple_venue_likes WHERE couple_id = %s",
                (couple_id,),
            )
            rows = cur.fetchall()
            cur.close()
    ...
    def search_semantic(
        ...
        return result.answer, self.extract_vendors_from_retriever(result)
    ...
    def extract_vendors_from_answer(self, answer: str) -> list[str]:
        ...
        for index, line in enumerate(lines[:-1]):
            ...
            if not current or not next_line.startswith("가격"):
                continue
    ...
    def build_vector_retrieval_query(
        self,
        category: str | None = None,
        region: str | None = None,
        max_price: int | None = None,
    ...
    count(rv) AS reviewCount"""
        ...
        if region:
            score_parts.append(
                f"CASE WHEN r.name CONTAINS '{region}' THEN 1 ELSE 0 END AS regionMatch"
            )
        if max_price:
            score_parts.append(
                f"CASE WHEN v.salePrice <= {max_price} AND v.salePrice > 0 THEN 1 ELSE 0 END AS priceMatch"
            )
        if min_price:
            score_parts.append(
                f"CASE WHEN v.salePrice >= {min_price} THEN 1 ELSE 0 END AS priceMinMatch"
            )

        if score_parts:
            parts.append("WITH v, score, r, tags, avgReviewScore, reviewCount,")
            parts.append(",\n    ".join(score_parts))
            score_names = [part.split(" AS ")[1] for part in score_parts]
            parts.append(
                f"""RETURN v.name AS name, v.category AS category,
    v.salePrice AS price, v.rating AS rating,
...
ORDER BY score DESC LIMIT 10"""
    ...
    def _get_schema(self) -> dict[str, Any]:
        ...
        with self.driver.session() as session:
            ...
            for record in nodes:
                ...
                schema["nodes"][label][key] = value_type
            for record in rels:
                start_label = record["sl"][0] if record["sl"] else "?"
                end_label = record["el"][0] if record["el"] else "?"
                schema["relations"].append(
                    f"(:{start_label})-[:{record['rt']}]->(:{end_label})"
                )
            return schema
    ...
    def _format_schema(self, schema: dict[str, Any]) -> str:
        ...
        for label, props in schema["nodes"].items():
            joined = ", ".join(f"{key}: {value}" for key, value in props.items())
    ...
    def _ensure_ready(self) -> None:
        if not self.ready():
            raise RuntimeError(
                ...
                + (self.startup_error or "Check OPENAI/Neo4j settings and installed packages.")
            )
